...
const EmployeeOrderPage = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get('http://localhost:3000/employees/getOrders', {
  ...
  return (
    <Box sx={{ padding: 2 }}>
      <Typography level="h2" sx={{ marginBottom: 2 }}>Employee Order Management</Typography>
      {orders.length === 0 ? (
        <Typography>No orders found.</Typography>
      ) : (
        <div>
          {orders.map((order) => (
            <Card key={order.orderId} variant="outlined" sx={{ marginBottom: 2 }}>
              <CardContent>
